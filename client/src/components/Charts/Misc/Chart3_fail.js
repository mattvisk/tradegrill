...
const Chart = ({chartData, tradeData, chartWidth, zoom, offset, setPremarketVol, setOpenToHighVol, setDayHigh})=>{    
    ...
    const init = () => {
        ...
        chartData.v.map((v, i) => {
            if(formatTime(chartData.t[i]) < '09:30'){
                ...
                setPremarketVol(premarketVolume);
                return;
            }
        });  
        ...
        chartData.t.map((e, i)=>{
            if(
                formatTime(chartData.t[i]) == '09:30' || 
                formatTime(chartData.t[i]) == '10:45' || 
                formatTime(chartData.t[i]) == '12:00' ||
                formatTime(chartData.t[i]) == '16:00') {
                c.lineWidth = 1;
                c.setLineDash([2, 3]);
        ...
        chartData.t.map((e, i)=>{
            { tradeData.map((e, x)=>{
                if(e.exec_time.substring(0, 5) >= formatTime(chartData.t[i]) && e.exec_time.substring(0, 5) < formatTime(chartData.t[i+1] )){
                    ...
                    c.rotate(((2 * Math.PI) / 6) * time.getSeconds() + ((2 * Math.PI) / 60) * time.getMilliseconds());
                    c.lineWidth = 1;
